...
class NeedlemanWunschSolver(object):
    ...
    def compute_cost_direction_matrices(self, a_seq: str, b_seq: str) -> (List[List[int]], List[List[Direction]]):
    ...
    def generate_initial_directions_matrix(self, a_seq_len: int, b_seq_len: int) -> List[List[Direction]]:
        return self.__generate_matrix(
            height=a_seq_len + 1,
            width=b_seq_len + 1,
    ...
    def __get_neighbor_costs(self, cost_matrix: List[List[int]],
                             a_seq: str, b_seq: str,
